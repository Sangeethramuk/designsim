...
async function generatePrototype(agentOutputs) {
  ...
  const prompt = ctx +
    ...
    '- Phone frame: white bg, border-radius:44px, box-shadow, black notch pill at top (120px wide).\n' +
    ...
    '- Include: status bar (9:41, wifi, battery), bottom nav (5 icons), header, cards, buttons.\n' +
    '- Content must match the specific product in the brief.\n' +
    ...
    '\n' +
    'Output ONLY the complete HTML document starting with <!DOCTYPE html>. Start immediately. No thinking.';

  try {
    const lead = AGENTS.find(a => a.id === 'lead');
    // Use higher max_tokens for prototype generation
    const protoAgent = { ...lead, brain: 'glm-latest' };
    const { model, messages, temperature } = buildMessages(protoAgent, prompt, []);
    // Override max tokens for prototype
    const protoMessages = messages.map(m => m.role === 'system' ? { ...m, content: m.content + '\n\nMAXIMUM OUTPUT BUDGET. Use it wisely — more HTML body, less CSS.' } : m);
    
    const raw = await callLLM({ ...lead, brain: 'glm-latest' }, prompt, []);
    const sanitized = sanitizeSwarmResponse(raw);
    ...
    // Check if body content is too small (CSS ate all the tokens)
    const bodyMatch = html.match(/<body>([\s\S]*)<\/body>/);
    const bodyContent = bodyMatch ? bodyMatch[1].replace(/<style[\s\S]*?<\/style>/gi, '').trim() : '';
    if (bodyContent.length < 500) {
      console.error('  Prototype body too small (' + bodyContent.length + ' chars) — CSS consumed all tokens');
      return null;
    }
